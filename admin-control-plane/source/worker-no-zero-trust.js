...
export async function latestPublishedConfig(env) {
  const row = await env.DB.prepare(
    "SELECT id, revision, payload, checksum, published_at FROM published_snapshots ORDER BY rowid DESC LIMIT 1",
  ).first();
  if (!row) return null;
  const payload = String(row.payload || "");
  const checksum = String(row.checksum || "");
  if (!timingSafeEqual(await sha256Hex(payload), checksum)) {
    throw new HttpError(500, "Published configuration integrity verification failed.", "storage_integrity_error");
  }
  let config;
  try {
    config = validateConfig(JSON.parse(payload));
  } catch {
    throw new HttpError(500, "Published configuration validation failed.", "storage_integrity_error");
  }
  return {
    id: String(row.id),
    revision: Number(row.revision),
...
async function publicConfigResponse(env, request) {
  if (!['GET', 'HEAD'].includes(request.method)) {
    throw new HttpError(405, "Method is not allowed.", "method_not_allowed", { Allow: "GET, HEAD" });
  }
  const published = await latestPublishedConfig(env);
  const body = JSON.stringify({ published });
  const etag = published ? `"${published.checksum}"` : '"empty"';
  if (request.headers.get("If-None-Match") === etag) {
    return new Response(null, { status: 304, headers: { ETag: etag, "Cache-Control": "public, max-age=60, stale-while-revalidate=300" } });
  }
...
async function handleApi(request, env, auth, requestId, url, origin) {
  ...
  if (url.pathname === "/api/health" && request.method === "GET") {
    ...
    return jsonResponse({ ok: true, sessionVerified: true, database: "reachable", auth: "password-plus-totp", publicIntegration: true });
  }
...
async function asset(env, request, pathname) {
  ...
  if (url.pathname === "/api/public-config") return publicConfigResponse(env, request);
