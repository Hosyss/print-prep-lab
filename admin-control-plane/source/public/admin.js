...
const viewCopy = {
  overview: ["Control plane overview", "Review protection, draft health and the read-only public publishing bridge."],
  ...
  release: ["Backups & safe snapshots", "Restore protected backups or publish a validated snapshot to the public site."],
...
function renderSelectedPage() {
  ...
  byId("page-source-note").textContent = catalogPage
    ? `Baseline: ${catalogPage.title}. Empty override fields leave the live baseline unchanged.`
...
async function publishSnapshot() {
  ...
  try {
    ...
    byId("publish-result").textContent = `Revision ${result.revision} published. The public site will refresh the approved content automatically.`;
    toast("Protected content published to Print Prep Lab.");
  } catch (error) {
