...
async function staticPageResponse(request: Request, env: Env, target: string): Promise<Response | null> {
  ...
  const assetRequest = new Request(new URL(target, request.url), {
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    /* Clean public URLs are canonical. The asset binding can still read these files
       internally, but direct .html duplicates are intentionally unavailable. */
    ...
    if (LEGACY_STATIC_ASSETS.has(url.pathname)) {
