...
try{
 ...
 await inputs.nth(0).fill('-25');await page.waitForTimeout(100);check('negative pixel width is clamped',Number(await inputs.nth(0).inputValue())>=1,await inputs.nth(0).inputValue());
 await inputs.nth(2).fill('0');await page.waitForTimeout(100);check('zero PPI is clamped',Number(await inputs.nth(2).inputValue())>=1,await inputs.nth(2).inputValue());
 const body=(await page.locator('body').innerText());check('invalid entry never renders NaN/Infinity',!/NaN|Infinity/.test(body));
 check('calculated results are labelled',body.includes('Calculated results'));
 ...
 const bleedInputs=page.locator('input[type="number"]');await bleedInputs.nth(0).fill('-1');await page.waitForTimeout(100);const bleedBody=await page.locator('body').innerText();check('negative trim never produces invalid result',!/NaN|Infinity/.test(bleedBody));
